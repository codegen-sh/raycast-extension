...
export class CodegenAPIClient {
  ...
  private async handleAPIError(response: Response): Promise<never> {
    ...
    if (response.status === 401) {
      await showCredentialsError(
        "Invalid API token. Please check your credentials in extension preferences.",
      );
  ...
  async getUser(organizationId: number, userId: number): Promise<UserResponse> {
